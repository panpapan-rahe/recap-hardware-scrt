...
def list_aktivitas_perangkat(perangkat_id: int, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    return get_aktivitas_by_perangkat(db, perangkat_id)


@router.get("/", response_model=list[AktivitasResponse])
def list_all_aktivitas(db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    return get_all_aktivitas(db)


@router.post("/", response_model=AktivitasResponse)
def add_aktivitas(data: AktivitasCreate, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    return create_aktivitas(db, data.model_dump(), user_id)
